...
v0.9.0 是“协议感知的薄代理”：解析 Handshake、Status Request/Ping 和 Login Start，完成域名选路、后端状态覆盖、自定义状态响应与玩家名白名单；连接后端后不终止 Minecraft 会话，后续 Login、Configuration、Play 与插件消息按原始字节透明转发。路由还可在 Minecraft 数据前写入 PROXY Protocol v1/v2，以便明确兼容的可信内网后端获取真实连接地址。
...
- 每后端活动连接、成功连接、尝试失败和连接延迟指标（v0.7.0 已完成）；主动健康检查仍待实现。
