...
v0.10.0 是“协议感知的薄代理”：解析 Handshake、Status Request/Ping 和 Login Start，完成域名选路、后端状态覆盖、自定义状态响应与玩家名白名单；连接后端后不终止 Minecraft 会话，后续 Login、Configuration、Play 与插件消息按原始字节透明转发。路由还可在 Minecraft 数据前写入 PROXY Protocol v1/v2，并通过可选主动 TCP 健康检查提前降低离线节点的选路优先级。
