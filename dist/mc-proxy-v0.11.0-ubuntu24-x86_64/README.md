...
- 可选主动健康检查，支持轻量 TCP 模式和 Minecraft Java `Handshake → Status → Ping/Pong` 协议模式，以及检查间隔、整次探测超时、连续失败/恢复阈值；健康节点优先，全部离线时仍保留真实连接降级尝试。
...
客户端无论使用哪个域名，都连接同一个 `settings.listen`。代理读取 Minecraft Java 握手里的域名并按 `rules` 出现顺序选择首个匹配路由，再按该路由的 `strategy` 选择后端；连接失败会尝试池内其余地址。启用 `[rules.health_check]` 后，即使没有玩家也会定时主动探测；达到失败阈值的节点排到健康/未知节点之后，全部节点离线时仍会逐个真实尝试，避免短暂抖动或探测误判造成硬中断。`mode = "tcp"` 只证明端口可达；`mode = "minecraft-status"` 会在单次超时内完成 Java Handshake、Status Request、基础 JSON 结构校验和 Ping/Pong，能拒绝 HTTP、空端口或卡死服务，但仍不执行登录、身份认证或模组协商。Status 探测 Host 默认使用精确路由 Host；通配路由或启用 `modify_virtual_host` 时使用具体后端主机，也可通过 `minecraft_host` 明确指定。探测兼容路由的 PROXY Protocol v1/v2。探测并发全局限制为 64，超出部分在后续调度周期继续。默认将玩家握手原文写入后端；若后端要求使用它自己的域名，设置 `modify_virtual_host = true`。`proxy_protocol = "v1"` 或 `"v2"` 只有后端监听器明确启用相同版本时才能使用，且必须用防火墙限制后端端口仅允许代理访问。存在 `[rules.status]` 时代理管理 Status Request 和 Ping：`custom` 完全生成响应，`backend` 拉取并覆盖后端响应；`whitelist_enabled = true` 时代理额外预读并原样保留 Login Start，只有白名单玩家才会继续连接后端。
...
`dist/` 包含在 Ubuntu 24.04 x86_64 原生编译并通过测试的 v0.11.0：

```sh
tar -xzf dist/mc-proxy-v0.11.0-ubuntu24-x86_64.tar.gz
chmod +x mc-proxy
./mc-proxy --help
```
